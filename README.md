...
(2)映射目录: 本地目录( `data` ) > 容器目录( `/usr/src/app/data` )。图片和内置 SQLite 数据库都会保存在该目录，请做好持久化和备份。

如需迁移旧 MySQL 数据，可在启动容器前将 `mysqldump` 生成的 `.sql` 文件放进该本地目录。容器会在启动时自动导入，并在导入成功后删除 SQL 文件；失败文件会保留。
